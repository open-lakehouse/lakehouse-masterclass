...
## The mental model: a stack of services, each in a container

A lakehouse is not one program. It's several independent services that run at once and cooperate over the network: an object store, a Spark cluster, a catalog and its database, and later Kafka and an orchestrator. That decomposition is deliberate, and it drives everything about how you run this. Each piece does one job with its own resource profile: computation is bursty and hungry for CPU and memory, storage is steady and wants disk, the catalog is a small always-on coordinator. Weld them into a single application and you can't scale or restart one without dragging the others along. Keep them separate and you can scale compute up for a heavy job while storage sits untouched, restart the catalog without stopping Spark, and swap one object store for another because services talk through stable interfaces, not shared internals. It also mirrors production, where these genuinely are separate systems, often on separate machines or managed services.

The question is how to run a stack of separate services on your laptop without turning your machine into the mess of version conflicts and half-uninstalled leftovers that installing them directly would create. The answer is **containers**. A container is a packaged, isolated copy of one piece of software with its own dependencies baked in, that runs the same way on any machine. Docker is what runs containers, and for most of this build it's the only thing that actually touches your host. The object store, Spark, the catalog, Kafka: each runs as its own container on top of Docker, so your machine stays clean and every reader ends up with the same stack.
...
Running one container by hand is a long command with a lot of flags. Running six of them, wired together with shared networks and consistent settings, is unmanageable that way. That's what **Docker Compose** solves. Compose describes your services in a single file, `docker-compose.yml`, one block per service, saying which image it runs, what ports it exposes, and how it connects to the others. One command brings the whole set up, another tears it down. The file is the source of truth: readable, kept in version control, and it *is* your infrastructure, written down rather than assembled from memory. That is the pattern for the rest of the course: each chapter adds one service to this Compose file, brings it up, and confirms it's healthy, so by the end you have the whole stack described in one file you wrote and understand line by line. This chapter just gets Docker and Compose in place so the next one can stand up the first real service.

![Each service block in the Compose file becomes one running container](../figures/ch02/fig-2.3-compose-to-stack.svg)

**Figure 2.3**. One block of text per service in `docker-compose.yml` becomes one running container. The file is your infrastructure: readable, versioned, and reproducible with one command. The next section is about the network these containers share: how they find and talk to each other.
...
When Compose brings up your services, it puts them all on a single private network it creates for the project. On that network, each service is reachable by its **service name**, the name of its block in the Compose file. So if you define a service called `seaweedfs`, another container can reach it at the address `seaweedfs`, and Compose's built-in DNS resolves that name to the right container. You do not use IP addresses, and you do not use `localhost`; you use the service name. This is why the Compose file's service names matter: they become the hostnames your services use to find each other.

Reaching a service from your **host** (your laptop, outside the container network) is a separate thing, and this is the distinction that causes the confusion. A container's ports are private to that Compose network unless you explicitly **publish** them. Publishing a port maps a port on your host to a port inside the container, written `ports: ["8333:8333"]` in the service definition, meaning host port 8333 forwards to container port 8333. Only then can you open `http://localhost:8333` in your browser or point a client on your machine at it. A service with no published ports still works fine for other containers; it's just invisible from your host.

So there are two ways to reach a service, and they use different addresses:

- **Container to container** (inside the stack): use the service name, for example `http://seaweedfs:8333`. This is how Spark will reach the object store.
- **Host to container** (from your laptop): use `localhost` and the published port, for example `http://localhost:8333`. This is how you open a web console or run a local client.
